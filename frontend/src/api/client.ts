...
export const adminApi = {
  ...
  discordIpCheck: () =>
    http.post<{ httpStatus: number; body: string | null; verdict: string; latencyMs: number }>('/admin/discord-ip-check').then((r) => r.data),
...
export const lootApi = {
  ...
  distribute: (raidId: number, lootId: number, memberIds: number[], divisor?: number) =>
    http.post<Loot[]>(`/raids/${raidId}/loots/${lootId}/distribute`, { memberIds, divisor }).then((r) => r.data),
